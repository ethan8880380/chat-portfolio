    ...
    <main className="min-h-screen bg-black">
      ...
      <section className="pt-32 pb-24 px-4 md:px-6">
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <div className="relative p-8 md:p-12">
              ...
              <div className="relative z-10">
                  ...
                  {contactInfo.map((item, index) => (
                    ...
                    >
                      ...
                      {item.href ? (
                        <a 
                          href={item.href}
                          className="text-xl text-white hover:text-[#0087ef] transition-colors font-medium"
                        >
                          {item.value}
                        </a>
                      ) : (
                ...
                <a 
                  href="mailto:ethan0380@gmail.com"
                  className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-white text-black font-semibold hover:bg-white/90 transition-all group"
                >
          ...
          >
            <h2 className="text-sm font-mono uppercase tracking-[0.2em] text-white/40 mb-6">
              Connect
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {socialLinks.map((item, index) => (
                <a
                  ...
                  className="flex items-center justify-between p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:border-[#0087ef]/30 hover:bg-[#0087ef]/5 transition-all group"
                >
                  <div className="flex items-center gap-4">
                    <div className="p-3 rounded-xl bg-white/5 group-hover:bg-[#0087ef]/20 transition-colors">
                      <item.icon className="w-5 h-5 text-white/50 group-hover:text-[#0087ef] transition-colors" />
                    </div>
                    <div>
                      <span className="font-semibold text-white group-hover:text-white transition-colors block">
                        {item.label}
                      </span>
                      <span className="text-sm text-white/40">{item.description}</span>
                    </div>
                  </div>
                  <ArrowUpRight className="w-5 h-5 text-white/30 group-hover:text-[#0087ef] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
                </a>
              ))}
            </div>
          </motion.div>
